...
Die Webseite soll sich fluide auf die Größe des Endgerätes anpassen.

* Die Seite soll in einem dezenten und angenehmen Farbschema erstellt werden  
...
Hierfür ist vorgesehen, dass Titel, Autor, Erscheinungsjahr, Auflage, Zustand, ISBN und Preis eingetragen werden. 
...
Nachdem die Webseite lokal (unter Verwendung von github) erstellt wurde, sollte diese komplett auf Heroku deployed werden.
Bei der Veröffentlichung von büchern.de gab es unterschiedliche Probleme. Ein Hauptproblem war es, dass wärend der Umsetzung eine SQLight Datenbank eingesetzt wurde, dies allerdings von Heroku nur minimal unterstützt wird. Aus diesem Grund kommt es hierbei sehr häufig zu Fehlern und die Datenbank wurde immer wieder gelöscht bzw. nicht mehr korrekt erkannt und somit skriptseitig neu Erstellt. Ein weiteres Problem ist es, dass Heroku Bilder nicht korrekt erkennt und beispielsweise hinter *.png Bilder oftmals ein "/" setzt und deshalb der Bildpfad nicht mehr stimmt. Bilder können dann nicht mehr gefunden und dargestellt werden. Allerdings kann der Code auf Heroku weder eingesehen noch bearbeitet werden und man kann das falsche Zeichen nicht entfernen. Auch auf der Registrieren-Seite macht Heroku Probleme bei der validierung des Formulars. So werden falsche Eingaben nicht rot markiert. Ansonsten können aber alle Funktionen von büchern.de (buechern.herokuapp.com) auf Heroku genutzt werden. 

###Ergebnis  
Das Ergebnis dieses Projekts ist ein nahezu fertiges Portal Büchern.de. Für einen tatsächlichen Go-Live müssten jedoch zunächst weitere Funktionalitäten entwickelt und eingebaut werden, welche es den Usern erleichtert Bücher zu verkaufen und zu kaufen. Diese können im letzten Punkt der Dokumentation eingesehen werden.
